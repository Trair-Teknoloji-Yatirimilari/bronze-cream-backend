...
export default function LogsTableClient({ initialData, totalCount, pageSize }: Props) {
  ...
  const fetchData = useCallback((pageNum: number) => {
    ...
    fetch(`/api/dashboard/logs?page=${pageNum}&pageSize=${pageSize}`, {
      headers: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        <DataTable
          columns={columns as unknown as import("@tanstack/react-table").ColumnDef<unknown, unknown>[]}
          data={data}
          loading={loading}
          onRefetch={refetch}
        />
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-sm text-muted-foreground">
          {total > 0 ? (
            <>
              <span className="font-medium">{startItem.toLocaleString('tr-TR')}</span>
              {" - "}
              <span className="font-medium">{endItem.toLocaleString('tr-TR')}</span>
              {" / "}
